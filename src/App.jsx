...
function App() {
    ...
    const [userRole, setUserRole] = useState(null) // null | 'COMERCIALES' | 'GERENTE'
    const [currentUser, setCurrentUser] = useState(null) // 'Ian', 'Pablo' etc.or null for Gerente

    // Dynamic Team Management - Persisted in LocalStorage
    const [salespeople, setSalespeople] = useState(() => {
        const saved = localStorage.getItem('salespeople_v1')
        return saved ? JSON.parse(saved) : ['Ian', 'Pablo', 'Belen', 'Lizeth', 'Formacion']
    })
    ...
    React.useEffect(() => {
        localStorage.setItem('salespeople_v1', JSON.stringify(salespeople))
    }, [salespeople])

    const manageTeam = {
        add: (name) => {
            if (name && !salespeople.includes(name)) {
                setSalespeople(prev => [...prev, name])
                alert(`Added ${name} to the team.`)
            }
        },
        remove: (name) => {
            if (confirm(`Remove ${name} from the team?`)) {
                setSalespeople(prev => prev.filter(p => p !== name))
            }
        }
    }
            ...
            <main style={{ paddingBottom: '4rem' }}>
                ...
                {userRole === 'GERENTE' && (
                    <div style={{ marginBottom: '2rem', padding: '1rem', border: '1px solid var(--color-border)', borderRadius: '0.5rem' }}>
                        <details>
                            <summary style={{ cursor: 'pointer', fontWeight: 'bold' }}>Manage Team Members</summary>
                ...
                */}
                ...
                <ContractList
                    ...
                    userRole={userRole}
                    onDelete={userRole === 'GERENTE' ? requestDelete : null}
